...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label, text, cache_image_features):
        ...
        if self.prompt_learner.training:
            ...
            return loss_xetext, loss_xeimage, logits_contrastive
...
class MaPLe(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            loss_xetext, loss_xeimage, logits_contrastive = model(image, label, text, cache_features)
            groundtruth = torch.arange(logits_contrastive.size(0), dtype=torch.long).to(self.device)
            loss_contrastive = F.cross_entropy(logits_contrastive, groundtruth)
            # print('loss_xetext:', loss_xetext)
            # print('loss_xeimage:', loss_xeimage)
            # print('loss_contrastive:', loss_contrastive)

            #Alternative: MMD Loss Implementation
            ...
            loss = (loss_xetext + loss_xeimage + loss_contrastive).sum()
            optim.zero_grad()
            loss.backward()
            optim.step()
            wandb.log({"loss": loss, "loss_contrastive ": loss_contrastive, "loss_xetext": loss_xetext, 'loss_xeimage':loss_xeimage})
