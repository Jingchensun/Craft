...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        if prompts is not None:
            ...
            # print("x", x.size()) #torch.Size([4, 77, 512])
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label, text, cache_image_features):
        ...
        text_batch_features = self.text_encoder(None, text)
        ...
        # print('logits:', logits.size()) #torch.Size([4, 50])
        loss_xetext =  F.cross_entropy(logits,label)
        # print('loss_xetext:', loss_xetext )
        logits_contrastive = image_features @ text_batch_features.t()
        # print('logits_contrastive:', logits_contrastive)
        if self.prompt_learner.training:
            ...
            # print('logits_xeimage:', logits_xeimage.size()) #torch.Size([4, 100])
            loss_xeimage = F.cross_entropy(logits_xeimage, label)
            # print('loss_xeimage:', loss_xeimage)
            ...
            return loss_xetext,loss_xeimage, text_features, fixed_embeddings, zero_shot_features, \
                   image_features, zero_shot_logits, logits_contrastive, logits
        else:
...
class PromptSRC(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            loss_ce, loss_xeimage, normalized_text_features, zs_clip_text_embeddings, zs_image_embedd, image_ft, \
            zero_shot_logits, logits_contrastive, logits = model(image, label, text, cache_features)
            # # MMD Loss Implementation
            # exp_logits = torch.exp(logits_contrastive)
            # # print('exp_logits:', exp_logits)
            # sum_exp_logits = torch.sum(exp_logits)
            # # print('sum_exp_logits:', sum_exp_logits)
            # lap_kernel  = exp_logits / sum_exp_logits
            # # print('lap_kernel:', lap_kernel )
            # c_diff = label.unsqueeze(1) != label.unsqueeze(0)
            # mmd_false = (lap_kernel)[c_diff == False].sum()
            # mmd_true = (lap_kernel)[c_diff == True].sum()
            # mmd_loss = mmd_false - mmd_true
            # mmd_loss = -torch.sum(torch.log(lap_kernel) * c_diff)
            # print('mmd_loss:',mmd_loss)
            # mmd_loss = torch.sum(torch.sum(lap_kernel * c_diff, dim=0), dim=0) / (torch.sum(c_diff) + 1e-6)
            ...
            loss.backward()
            optim.step()
            #wandb.log({"loss": loss, "mmd_loss ": mmd_loss, "loss_xetext": loss_ce, 'loss_xeimage':loss_xeimage, "L_SCL": L_SCL})
